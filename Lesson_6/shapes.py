...
class Triangle(Parallelogram):
    def __init__(self, x, y, height, width, angle):
        ...
        self.third_side = math.sqrt(
            self.height ** 2 + self.width ** 2 - (2 * (self.height * self.width) * math.cos(self.angle)))

    def square(self):
        pi = (self.height + self.width + self.third_side) / 2
        return math.sqrt(pi * (pi - self.width) * (pi - self.height) * (pi - self.third_side))
...
t = Triangle(1, 2, 10, 20, 45)
t1 = Triangle(1, 2, 40, 40, 45)
